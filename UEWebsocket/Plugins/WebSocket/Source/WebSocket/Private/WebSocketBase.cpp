...
void UWebSocketBase::SendBinary(int length)
{
	if (length > MAX_ECHO_PAYLOAD)
	{
		UE_LOG(WebSocket, Error, TEXT("too large package to send > MAX_ECHO_PAYLOAD:%d > %d"), length, MAX_ECHO_PAYLOAD);
		return;
	}
...
void UWebSocketBase::ProcessWriteable()
{
	...
	while (mSendQueueBinary.Num() > 0)
	{
		unsigned char buf[LWS_PRE + MAX_ECHO_PAYLOAD];

		memcpy(&buf[LWS_PRE], mSendQueueBinary[0].GetData(), mSendQueueBinary[0].Num());
		lws_write(mlws, &buf[LWS_PRE], mSendQueueBinary[0].Num(), LWS_WRITE_BINARY);

		mSendQueueBinary.RemoveAt(0);
	}
}
